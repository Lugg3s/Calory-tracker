...
To keep onboarding short, the current working direction maps one coarse everyday-activity choice to an internal 8-hour reference profile. Five provisional profiles are documented in [`activity-model-v1.md`](activity-model-v1.md): predominantly seated, mixed sitting/standing, predominantly standing, physically active and heavy physical work.

These profiles are provisional product parameters. Known limitations are documented: low-activity users may have some NEAT undercounted, while high occupational activity can partly overlap with step energy. V1 proceeds with this model for now and should be calibrated later against real data.
...
`0.57 kcal/kg/km` is the current scientifically supported working value for net walking cost. `0.414 × height` is only a fallback estimate; measured distance from phone/Health/wearable data should later take precedence.
...
Weekly training energy is divided by 7 for the daily-average plan. This is why typical training duration is a conditional onboarding input. A separate intensity question is not required for V1; standard MET values per sport can be used initially.
...
When intake is below maintenance, the lower TEF at lower intake is handled explicitly in the target-calorie equation rather than assuming maintenance-level TEF remains unchanged.

## Target KFA → target weight

Target KFA does not replace target weight in the downstream calculation. It is an alternative way to derive the target weight when current KFA is known:
...
This is the V1 target-calorie equation.

## Automatic-plan guardrails

The upper restriction is **maintenance-relative**, not a fixed kcal deficit.
...
Because TEF is handled separately, the corresponding maximum planned body-energy deficit is:

```text
D_max = 0.225 × M
```

If the requested goal/timeframe exceeds this, the app should propose a longer timeframe rather than silently recommending a more restrictive plan.

Additional automatic-plan floors remain approximately:
...
The weekly-budget calculation is now defined:

```text
weekly budget W = average daily target C × 7
```

A planned higher-calorie/flexible day **does not add calories on top**. It redistributes the same weekly budget.
...
V1 automatic planning uses **maintenance calories as the maximum flexible-day budget**. The app does not automatically plan a calorie surplus on that day.

If redistribution would push regular days below the automatic-plan guardrails, the app should reduce the flexible-day budget or propose a longer timeframe.

The exact screen copy, terminology (“Cheat Day” vs. “flexibler Tag” / “höheres Tagesbudget”), day selector and interaction design remain open.

## Manual calorie override

The automatically calculated calorie target is editable in settings/an advanced area, not prominently on the main screen. How to handle manual values below automatic-plan guardrails remains open.

## Tracking modes and macros

Simple mode: calories only.

Advanced mode: calories + protein + fat + carbohydrates.

Current macro defaults remain provisional:

- 1.2 g protein/kg/day without regular strength training
- 2.0 g/kg/day for regular strength/performance training
- 0.8 g fat/kg/day as provisional floor
- carbohydrates fill remaining calories

These must be scientifically validated before hard-coding.
