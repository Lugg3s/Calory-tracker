...
This reference comes from a user-provided onboarding UI example. It should be used as a visual and interaction reference when designing Theo's onboarding screens.
...
This matches the existing Theo onboarding principle of one primary input parameter per screen.
...
The reference places a short explanatory sentence directly underneath the main question. For Theo, this information should **not** be permanently visible.
...
For Theo, the height screen should remain focused on the height input itself. A human figure should not be added merely to fill the available space or make the screen visually richer.
...
For Theo, the current-weight screen should therefore prioritize only the weight input in its default state.
...
- **Explain relevance, not just definitions.** Optional help should say why the input matters and how Theo uses it.
